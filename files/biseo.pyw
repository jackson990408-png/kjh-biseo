...
.hitem.cur{background:#E7E3D5}
.hitem .ht{flex:1;min-width:0;font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
<div id="qmodal" class="overlay"><div class="box">
  ...
  <input id="qcmd" placeholder="실행할 명령 (예: 스케치업 열어줘)">
...
function renderQ(){
  ...
  qbtns.forEach(function(b,i){
    ...
    t.onclick=function(){inp.value=b.cmd;doSend()};
...
document.getElementById('featbtn').onclick=openFeat;
...
document.getElementById('namebtn').onclick=openNameModal;
...
function hideUpdate(){document.getElementById('updbanner').classList.remove('show')}
...
class Api:
    ...
    def do_update(self, download_url):
        """업데이트 파일을 다운받아 교체 bat 를 실행하고 앱을 재시작한다."""
        try:
            ...
            # cp949: Windows cmd 는 기본 코드페이지 사용
